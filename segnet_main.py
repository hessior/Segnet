'''
Segnet main module, for cCT.
Trained with weighted loss(abs loss), more weights for bone.
'''
...
#### local packages
import segnet_model as segnet
from img_trans import data_aug

##############   18/03/26  it worked

trainPATH = r"../train_T1.npy"
testPATH = r"../test.npy"
IMGtoPATH = "./my_output_trans-{}/"
testIMGtoPATH = "./generate_{}/"
CheckPointPATH = "./ckpt_trans/my_model.ckpt"
SummaryPATH = "./summary_segnet.txt"
...
def train():
    ...
    logits = segnet.Segnet(imgs, is_training, "segnet")
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(from_epoch, from_epoch + EPOCH):
            ...
            ## train the model 
            lrate = LEARNING_RATE if epoch < 40 else LEARNING_RATE*(
                    from_epoch + EPOCH - epoch)/(from_epoch + EPOCH - 40)
